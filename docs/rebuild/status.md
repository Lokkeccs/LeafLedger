...
| P1-WP04 | OpenAPI → generated TS client pipeline | done | [plan](./plans/P1-WP04-openapi-client-pipeline.md) — QA **PASS** 2026-07-06. Build-time OpenAPI doc → committed `backend/openapi/leafledger-v1.json`; `openapi-typescript`+`openapi-fetch` → `app/src/api/`; `getMeta()` + vitest; CI `contract` diff gate (red-gate independently reproduced). Release 0/0; arch 3/3; lint/typecheck/test/page-budget green; 0 vulns. Ready for LL Git (branch P1-WP04; 16-path set). |
