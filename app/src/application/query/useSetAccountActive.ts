...
import { setAccountActive } from '../accounts'
import { qk } from './queryKeys'

export function useSetAccountActive(spaceId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ accountId, active, idempotencyKey }: { accountId: string; active: boolean; idempotencyKey?: string }) => setAccountActive(spaceId, accountId, active, idempotencyKey),
    throwOnError: false,
