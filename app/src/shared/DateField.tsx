...
export function DateField({ id, label, error, ...props }: DateFieldProps) {
  return <label htmlFor={id} style={{ display: 'grid', gap: 'var(--space-1)' }}>
    ...
    <input {...props} id={id} type="date" aria-invalid={error ? true : undefined} aria-describedby={error ? `${id}-error` : undefined} style={{ padding: 'var(--space-2) var(--space-3)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)', color: 'var(--color-text)', ...props.style }} />
