...
export function FormField({ id, label, error, description, control, ...inputProps }: FormFieldProps) {
  ...
  const fieldControl = control
    ...
    : <input {...inputProps} id={id} aria-invalid={error ? true : undefined} aria-describedby={describedBy} style={{ padding: 'var(--space-2) var(--space-3)', border: '1px solid var(--color-line)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)', color: 'var(--color-ink)', ...inputProps.style }} />

  return <label htmlFor={id} style={{ display: 'grid', gap: 'var(--space-1)' }}>
    <span style={{ fontWeight: 700, fontSize: 13 }}>{label}</span>
    {fieldControl}
    {description && <span id={`${id}-description`} style={{ color: 'var(--color-muted)', fontSize: 13 }}>{description}</span>}
